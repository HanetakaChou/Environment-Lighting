...
#include "support/DDSTextureLoader.h"
#include "demo.h"
#include "../dxbc/integrate_form_factor_normal_distribution.inl"
#include "../dxbc/integrate_ld_normal_distribution.inl"
#include "../dxbc/integrate_hemispherical_directional_reflectance.inl"
...
void Demo::Init(ID3D11Device *d3d_device, ID3D11DeviceContext *d3d_device_context, IDXGISwapChain *dxgi_swap_chain)
{
	...
	this->m_distant_radiance_distribution = NULL;
	this->m_distant_radiance_distribution_srv = NULL;
	{
		ID3D11Resource *tmp = NULL;

		HRESULT res_create_dds_texture_from_file = DirectX::CreateDDSTextureFromFileEx(d3d_device, L"assets/distant_radiance_distribution.dds", 0U, D3D11_USAGE_DEFAULT, D3D11_BIND_SHADER_RESOURCE, 0U, 0U, true, &tmp, &this->m_distant_radiance_distribution_srv);
		assert(SUCCEEDED(res_create_dds_texture_from_file));

		HRESULT res_query_interface = tmp->QueryInterface(IID_PPV_ARGS(&this->m_distant_radiance_distribution));
		assert(SUCCEEDED(res_query_interface));

		tmp->Release();
	}

	this->m_integrate_form_factor_normal_distribution_cs = NULL;
	{
		HRESULT res_d3d_device_create_compute_shader = d3d_device->CreateComputeShader(code_shader_integrate_form_factor_normal_distribution, sizeof(code_shader_integrate_form_factor_normal_distribution), NULL, &this->m_integrate_form_factor_normal_distribution_cs);
	...
	this->m_form_factor_normal_distribution = NULL;
	{
		...
		d3d_buffer_desc.ByteWidth = sizeof(float) * 4U * 7U;
	...
	this->m_form_factor_normal_distribution_uav = NULL;
	{
		...
		d3d_unordered_access_view_desc.Buffer.NumElements = 4U * 7U;
	...
	this->m_form_factor_normal_distribution_srv = NULL;
	{
		...
		d3d_shader_resource_view_desc.BufferEx.NumElements = 4U * 7U;
...
void Demo::Tick(ID3D11Device *d3d_device, ID3D11DeviceContext *d3d_device_context, IDXGISwapChain *dxgi_swap_chain)
{
	...
	// integrate form factor normal distribution
	{
		d3d_device_context->CSSetShader(this->m_integrate_form_factor_normal_distribution_cs, NULL, 0U);
		d3d_device_context->CSSetShaderResources(0U, 1U, &this->m_distant_radiance_distribution_srv);
		...
		d3d_device_context->Dispatch(1U, 1U, 1U);
	}
