...
#include "../thirdparty/Environment-Lighting/shaders/brx_diffuse_environment_lighting.bsli"
...
float3 EvaluateEnvironmentLighting(float3 diffuse_color, float3 specular_color, float roughness, float3 N, float3 V, float raw_ambient_occlusion)
{
    ...
    float3 diffuse_radiance = brx_diffuse_environment_lighting(diffuse_color, specular_color, N, irradiance_coefficients);
