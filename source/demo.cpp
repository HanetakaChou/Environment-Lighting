...
void Demo::Init(ID3D11Device *d3d_device, ID3D11DeviceContext *d3d_device_context, IDXGISwapChain *dxgi_swap_chain)
{
	...
	this->m_environment_equirectangular_map_srv = NULL;
	{
		...
		HRESULT res_create_dds_texture_from_file = DirectX::CreateDDSTextureFromFileEx(d3d_device, L"../thirdparty/HDRI-The-Sky-Is-On-Fire/hdri-the-sky-is-on-fire-equirectangular.dds", 0U, D3D11_USAGE_DEFAULT, D3D11_BIND_SHADER_RESOURCE, 0U, 0U, true, &tmp, &this->m_environment_equirectangular_map_srv);
	...
	{
		this->m_environment_octahedral_map_width = std::max(this->m_environment_equirectangular_map_width, this->m_environment_equirectangular_map_height) / 2U;
		this->m_environment_octahedral_map_height = std::max(this->m_environment_equirectangular_map_width, this->m_environment_equirectangular_map_height) / 2U;
