...
#include "../thirdparty/Environment-Lighting/include/brx_environment_lighting_sh_projection_reduce.h"
...
void Demo::Init(ID3D11Device *d3d_device, ID3D11DeviceContext *d3d_device_context, IDXGISwapChain *dxgi_swap_chain)
{
	...
	this->m_environment_equirectangular_map_srv = NULL;
	{
		...
		HRESULT res_create_dds_texture_from_file = DirectX::CreateDDSTextureFromFileEx(d3d_device, L"../assets/environment-map.dds", 0U, D3D11_USAGE_DEFAULT, D3D11_BIND_SHADER_RESOURCE, 0U, 0U, true, &tmp, &this->m_environment_equirectangular_map_srv);
...
void Demo::Tick(ID3D11Device *d3d_device, ID3D11DeviceContext *d3d_device_context, IDXGISwapChain *dxgi_swap_chain)
{
	// sh project equirectangular map
	{
		...
		UINT const tile_num_width = static_cast<UINT>(static_cast<float>(this->m_environment_equirectangular_map_width + BRX_ENVIRONMENT_LIGHTING_SH_PROJECTION_THREAD_GROUP_X - 1) / static_cast<float>(BRX_ENVIRONMENT_LIGHTING_SH_PROJECTION_THREAD_GROUP_X));
		UINT const tile_num_height = static_cast<UINT>(static_cast<float>(this->m_environment_equirectangular_map_height + BRX_ENVIRONMENT_LIGHTING_SH_PROJECTION_THREAD_GROUP_Y - 1) / static_cast<float>(BRX_ENVIRONMENT_LIGHTING_SH_PROJECTION_THREAD_GROUP_Y));
	...
	// sh project octahedral map
	{
		...
		UINT const tile_num_width = (this->m_environment_octahedral_map_width + BRX_ENVIRONMENT_LIGHTING_SH_PROJECTION_THREAD_GROUP_X - 1) / (BRX_ENVIRONMENT_LIGHTING_SH_PROJECTION_THREAD_GROUP_X);
		UINT const tile_num_height = (this->m_environment_octahedral_map_height + BRX_ENVIRONMENT_LIGHTING_SH_PROJECTION_THREAD_GROUP_Y - 1) / (BRX_ENVIRONMENT_LIGHTING_SH_PROJECTION_THREAD_GROUP_Y);
